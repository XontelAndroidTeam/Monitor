...
public class MediaPlayer extends VLCObject<MediaPlayer.Event> {
    ...
    private static final int MIN_SPACE_TO_CREATE_RECORD_FILE = 1000000;
...
//        public static final int ChapterChanged      = 0x11d;
    ...
    public Bitmap takeSnapShot(Context context) {
        if (mVideoHelper != null) {
            return mVideoHelper.takeSnapShot(context);
        } else {
            Toast.makeText(context, R.string.cant_take_photo, Toast.LENGTH_SHORT).show();
        }
        return null;
    }
    ...
    public void unMute(){
        setVolume(Math.max(volumeLevel, getVolume()));
    }
    ...
    public void startRecording() {
        if (!recording /* is not in recording state */ && recordingDirectory != null) {
            long diskSpace = new File(recordingDirectory).getFreeSpace();
            Log.v("[Media player]", "Available disk space : " + diskSpace);
            if (diskSpace > MIN_SPACE_TO_CREATE_RECORD_FILE) { // check if can create a file for record writing
                recording = nativeRecord(recordingDirectory);
                if (recording) {
                    mVideoHelper.setRecordViewEnabled(true);
                } else {
                    Toast.makeText(context, R.string.error_occurred_while_processing, Toast.LENGTH_SHORT).show();
                }
            } else {
               AndroidUtil.showAlertDialog(context, R.string.no_space_left, R.string.record_stopped);
            }
        } else {
            Log.v("[Media player]", "recording directory is not specified or is currently recording");
        }
    }
