...
import org.videolan.libvlc.util.VolumeControllerView;
...
class VideoHelper implements IVLCVout.OnNewVideoLayoutListener {
    ...
    private MediaPlayer.ScaleType mCurrentScaleType = MediaPlayer.ScaleType.SURFACE_FILL;
    ...
    private void init(MediaPlayer player, VLCVideoLayout surfaceFrame, DisplayManager dm, boolean subtitles, boolean useSurfaceView) {
        mMediaPlayer = player;
        mDisplayManager = dm;
        recordPanel = surfaceFrame.findViewById(R.id.ll_record_panel);
        stopRecord = surfaceFrame.findViewById(R.id.btn_stop);
        timerText = surfaceFrame.findViewById(R.id.timer);
        volumeControllerView = surfaceFrame.findViewById(R.id.volume_controller);
        setupVolumeView();
        stopRecord.setOnClickListener(view -> {
            mMediaPlayer.stopRecording();
            Toast.makeText(mVideoTexture.getContext(), R.string.record_saved, Toast.LENGTH_SHORT).show();
        });
        ...
        if (isPrimary) {
            ...
            loadingDots = mVideoSurfaceFrame.findViewById(R.id.loading_dots);
            errorTextView = mVideoSurfaceFrame.findViewById(R.id.tv_error);
            if (useSurfaceView) {
    ...
    private void setupVolumeView() {
        btnSound = volumeControllerView.findViewById(R.id.iv_sound);
        btnSound.setVisibility(mMediaPlayer.getVolume() > 0 ? View.VISIBLE : View.GONE);
        btnSound.setOnClickListener(view -> {
            int volume = mMediaPlayer.getVolume();
            if(volume > 0){
                mMediaPlayer.mute();
                btnSound.setImageDrawable(view.getContext().getDrawable(R.drawable.ic_baseline_volume_off_24));
            }else{
                mMediaPlayer.unMute();
                btnSound.setImageDrawable(view.getContext().getDrawable(R.drawable.ic_baseline_volume_up_24));
            }
        });
    }
    ...
    public Bitmap takeSnapShot(Context context) {
        if (mMediaPlayer != null && mMediaPlayer.hasMedia() && mMediaPlayer.isPlaying() && mVideoTexture != null) {
            return mVideoTexture.getBitmap();
        } else {
            Toast.makeText(context, context.getString(R.string.no_media_available), Toast.LENGTH_SHORT).show();
        }
        return null;
    }
